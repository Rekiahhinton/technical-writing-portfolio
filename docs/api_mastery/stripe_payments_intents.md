...
| ``processing`` | Indicates the PaymentIntent has been confirmed and the payment is still processing. |
...
```
Copy the ``id`` key from the JSON response for the next step. It begins with ``pm_``. 
...
**Note:** If the PaymentIntent is configured to accept payment methods that cause redirects, Stripe requires a ``return_url``. In this quickstart guide, adding a placeholder ``return_url`` to the body parameters is the easiest way to prevent an error. Add it to the ``Body`` tab after selecting ``x-www-form-urlencoded``:
...
| ``5200 8282 8282 8210`` | Mastercard (Debit) | Simulate by Country | ``succeeded`` |
| ``5555 5582 6555 4449`` | Mastercard (United Kingdom) | Simulate by Country | ``succeeded`` |
