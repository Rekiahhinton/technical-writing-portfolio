...
By the end of this guide, developers will be able to build apps with access to a Spotify user's listening history, playlists, saved tracks, and more. 
...
2. A Registered Spotify App - [Create an App](https://developer.spotify.com/documentation/web-api/concepts/apps) (Includes your Client ID, Client Secret, and redirect URI configuration)
...
The Authorization URL includes the Spotify authorization endpoint along with the following parameters, ``client_id``, ``response_type``, ``redirect_uri``, ``scope``, and ``state``:
